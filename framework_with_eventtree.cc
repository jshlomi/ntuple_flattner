...
int main(int argc, char* argv[]) {
  ...
  vector<float>* jet_ip2d_llr = new vector<float>;
  vector<float>* jet_ip3d_llr = new vector<float>;
  chain->SetBranchAddress("jet_ip2d_llr", &jet_ip2d_llr);
  chain->SetBranchAddress("jet_ip3d_llr", &jet_ip3d_llr);
  ...
  int numBjets, numCjets, numUjets;
  eventTree->Branch("numBjets",&numBjets,"numBjets/I");
  eventTree->Branch("numCjets",&numCjets,"numCjets/I");
  eventTree->Branch("numUjets",&numUjets,"numUjets/I");
  ...
  inputVarsMap["pt"]                          =  &jet_pt_orig_jet_i;
  inputVarsMap["abs(eta)"]                    =  &jet_eta_orig_abs_jet_i;
  inputVarsMap["jet_pt_jet_i"]                =  &jet_pt_orig_jet_i;
  inputVarsMap["jet_eta_jet_i"]               =  &jet_eta_orig_abs_jet_i;
  inputVarsMap["jet_pt_orig_jet_i"]           =  &jet_pt_orig_jet_i;
  inputVarsMap["jet_eta_orig_abs_jet_i"]      =  &jet_eta_orig_abs_jet_i;
  ...
  if(DumpRawData ==1){
      outputFile.open( "raw_data_framework.txt" );
      outputFile2.open( "raw_vars_data_framework.txt" );
      outputFile3.open( "raw_sv1_dR_data_framework.txt" );    
  }   
  ...
  for(int evt_i=0; evt_i < n_entries_chain; evt_i++){
	...
	jet_mv2c00_jet_i=(*jet_mv2c00)[jet_i];
        ...
        TVector3 jetAxis;
        jetAxis.SetPtEtaPhi(jet_pt_orig_jet_i,jet_eta_orig_jet_i,jet_phi_orig_jet_i);
        ...
        if (sv1_vtx_ok) {
          ...
          if(DumpRawData==1){
            outputFile3 << evt_i << "\t" << jet_i << "\t"<< jetAxis.X() << "\t" << jetAxis.Y() << "\t" << jetAxis.Z() << "\t" << pv2sv.X() << "\t"<< pv2sv.Y() << "\t"<< pv2sv.Z() << endl;
          }
        ...
        if (!sv1_vtx_ok) {
          sv1_ntkv = -1;
          sv1_n2t  = -1;
          sv1_mass = -1e3; //original -1e3
          sv1_efrc = -1; //original -1;  //mine: -0.2;
          sv1_sig3 = -100; //original -100
        }
        ...
        if ( not(jf_mass>0) ){
          ...
          jf_nvtx  =-1;
          jf_nvtx1t=-1;
          jf_mass  = -1e3; //original -1e3
          jf_efrc  = -1; //original: -1;  //mine: -0.2
          jf_sig3  = -100;  //original: -100; //mine: -40
          jf_dR    = -1; //original: -1; //mine: -0.2
        }   
        ...
        JF_Lxy1 = (closestVtx_L3D > 0 ) ? closestVtx_L3D*sin(jf_theta) : -5;
        JF_Lxy2 = (Second_closestVtx_L3D > 0 ) ? Second_closestVtx_L3D*sin(jf_theta) : -5;
	...
	mv2c00 = jet_mv2c00_jet_i;
        ...
        if(jet_pt_jet_i > 800e3){ jet_pt_jet_i = 800e3; }

        //if(ip2 > 30){ ip2 = 30; }
        //if(ip2 < -15){ ip2 = -15; }
        
        //if(ip2_c > 20){ ip2_c = 20; }
        //if(ip2_c < -15){ ip2_c = -15; }
       
        //if(ip2_cu > 20){ ip2_cu = 20; }
        //if(ip2_cu < -15){ ip2_cu = -15; }
        
        //if(ip3 > 35){ ip3 = 35; }
        //if(ip3 < -15){ ip3 = -15; }

        //if(ip3_c > 20){ ip3_c = 20; }
        //if(ip3_c < -15){ ip3_c = -15; }

        //if(ip3_cu > 25){ ip3_cu = 25; }
        //if(ip3_cu < -15){ ip3_cu = -15; }

        //if(sv1_n2t > 40){ sv1_n2t = 40; }
        
        //if(sv1_Lxy > 100){ sv1_Lxy = 100; }
        //if(sv1_Lxy < -50){ sv1_Lxy = -50; }

        //if(sv1_L3d > 200){ sv1_L3d = 200; }
   
        //if(sv1_sig3 > 400){ sv1_sig3 = 400; }

        //if(sv1_dR > 6){ sv1_dR = 6; }

        //if(jf_n2tv > 50){ jf_n2tv = 50; }
        
        //if(jf_dR > 0.6){ jf_dR = 0.6; }

        if(JF_Lxy1 > 60){ JF_Lxy1 = 60; }
        if(JF_Lxy1 < -5){ JF_Lxy1 = -5; }
        if(JF_Lxy2 > 60){ JF_Lxy2 = 60; }
        if(JF_Lxy2 < -5){ JF_Lxy2 = -5; }

        if(e_first_vtx > 1.5e5){ e_first_vtx = 1.5e5; }
        if(e_second_vtx > 1e5){ e_second_vtx = 1e5; }
        if(e_first_vtx == 0  ){ e_first_vtx = -100; }
        if(e_second_vtx == 0 ){ e_second_vtx = -100; }

        if(mass_first_vtx > 5000){ mass_first_vtx = 5000; }
        if(mass_second_vtx > 4000){ mass_second_vtx = 4000; }
        ...
        if(closestVtx_L3D > 120){ closestVtx_L3D = 120; }
        if(Second_closestVtx_L3D > 120){ Second_closestVtx_L3D = 120; }
...
if(DumpRawData && number_of_Readers > 1){
  outputFile << evt_i << "\t" << jet_i << "\t"<< mv2new_1 << "\t"<< mv2new_2 << endl;
 outputFile2 << evt_i << "\t" << jet_i << "\t" << jet_pt_jet_i << "\t" << jet_eta_jet_i << "\t" << ip2 << "\t" << ip2_c << "\t" << ip2_cu << "\t" <<
 ip3 << "\t" << ip3_c << "\t" << ip3_cu << "\t" << sv1_ntkv << "\t" << sv1_mass << "\t" << sv1_efrc << "\t" << sv1_n2t << "\t" << sv1_Lxy << "\t" << sv1_L3d << "\t" <<
 sv1_sig3 << "\t" << sv1_dR << "\t" << jf_n2tv << "\t" << jf_ntrkv << "\t" << jf_nvtx << "\t" << jf_nvtx1t << "\t" << jf_mass << "\t" << jf_efrc << "\t" <<
 jf_dR << "\t" << jf_sig3 << endl;
}
